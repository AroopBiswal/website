...
- **Work/Projects**: no internal scrollers any more — the sections are full-height blocks in the page scroll. **Editorial restyle (Sep 2026)**: both share one 960px column; every card/row is a square-cornered, borderless block in the panel fill (hover warms the fill 8% toward ink and lifts 2px — no shadow, no tint, both were tried and taken off); role, period, project number and date are 11px tracked uppercase labels with tabular figures; project tags are small pills with a 7% ink wash; link rows show an outward `.proj-arrow` on hover (always on mobile). The old `.period-pill` and the inline styles in `site.tsx` went away in favour of the classes above, so further variations are CSS-only.
...
| Date | Change |
|---|---|
| Sep 2026 | **Work and Projects restyled** as quiet editorial blocks: square corners, no borders, panel fill, tracked small labels, tabular figures, pill tags, one shared 960px column, hover = warm fill + lift. Tried and rejected on the way: hairline borders, a blue top rule on the featured card, a blue-tinted featured fill, and a hover shadow. See Behaviors → Work/Projects. |
