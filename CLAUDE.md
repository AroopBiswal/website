...
| Sep 2026 | **Dashboard analytics rewrite** in `next.config.ts`: `/2289c7240943bbb3/:path*` proxies to the Bull and Bear dashboard project. Vercel Web Analytics requests its script and beacon from the **site root**, not from under `/trading`, so they were landing on this project and the dashboard recorded nothing. The path is an obfuscated per-project prefix Vercel generates so ad blockers cannot match `/_vercel/insights`; it is unique to that project and does not touch this site's own analytics. The failure mode is silent (script 404s, the pageview sits unsent in `window.vaq`), and the hash is Vercel-generated with no stability guarantee, so check it first if dashboard analytics goes quiet. |
