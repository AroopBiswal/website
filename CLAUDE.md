...
`.site-root` / `.site-rule` / `.site-header` / `.site-nav`(+`.site-nav-right`) / `.site-content` / `.scroll-view` / `.section`(+`.section-centered`) (shell) · `.navbtn`(+`.active`) · `.nav-icon` (the camera link to `/photos`, same round look as the toggle, `.active` fills it blue) · `.theme-toggle` (38px round icon button; `.theme-icon-sun`/`-moon` both rendered, `html[data-theme]` picks one in CSS so hydration never mismatches) · `.eye`/`.pupil` · `.sticker` (big button w/ shadow, `--tilt` var controls hover rotation) · `.chip-btn` (small) · `.card` / `.work-card` / `.job-bullets` · `.project-row` / `.proj-date` · `.about-scroll`/`.about-section` (scroll-snap) · `.interest-chip` · `.dot`
...
- Nav has two external links, both `target="_blank"`: **GitHub** (github.com/AroopBiswal) in the left group, and **Trading** (`https://aroopbiswal.com/trading` — the absolute URL, not the `/trading` rewrite path) in the right group. The right group runs Trading · Blog · Alligator · About Me, then two round icon buttons: a **camera** linking to `/photos` (`.nav-icon`, aria-label "Photography", lit blue while on that page) and the theme toggle.
