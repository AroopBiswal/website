...
`.site-root` / `.frame-top`(+`.frame-top-line`) / `.site-gutter-top` / `.site-nav` / `.site-content` (shell) · `.section`(+`.section-center`, `.section-split`) · `.scroll-cue` · `.navbtn`(+`.active`) · `.theme-toggle` · `.eye`/`.pupil` · `.sticker` (big hero button w/ hard shadow, `--tilt` var controls hover rotation) · `.chip-btn` (small hero button) · `.card` / `.work-card` · `.project-row` · `.contact-link` · `.about-intro` · `.interest-chip` · `.dot` / `.about-dots`
...
Sections are `min-height: 100dvh` and grow when content is taller (Work does). Work and Projects use
`.section-split`: a two-column grid (300px title rail + content) capped at 1320px and centered — the
`.panel-head` is `position: sticky` so the title holds under the nav while the cards scroll past. This
exists because a left-pinned title over centered cards fell apart on ultra-wide screens. On ≤860px the
split stacks back to a column and the title unsticks. Anchors work natively
...
| Aug 2026 | **Sticky two-column sections** (same branch, user-picked from options): Work/Projects titles moved into a 300px left rail (`.section-split` grid, capped 1320px, centered) with `position: sticky` under the nav; cards/list fill the right column. Fixes the ultra-wide disconnect between left-pinned titles and centered cards. `project-row` columns tightened (56/1fr/320/180) for the narrower right column; stacks + unsticks ≤860px. |
